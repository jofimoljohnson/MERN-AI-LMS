...
const SearchWithAi = () => {
    ...
    const handleSearch = async () => {
        ...
        recognition.onresult = async (e) => {
            ...
            setInput(transcript);
            await hanldeRecommendation(transcript);
        };
    };

    const hanldeRecommendation = async (query) => {
        try {
            const result = await api.post("/api/course/search", { input: query });
            ...
            <div className="min-h-screen bg-gradient-to-br from-black to-gray-900 text-white flex flex-col items-center px-4 py-16">
                ...
                <div className="bg-white shadow-xl rounded-3xl p-6 sm:p-8 w-full max-w-2xl text-center relative">
                    <FaArrowLeftLong
                        ...
                        onClick={() => navigate("/")}
                    />
                    <h1 className="text-gray-600 text-2xl sm:text-3xl font-bold  mb-6 flex items-center justify-center gap-2">
                        <img src={aiimg} alt="" className="w-8 h-8 sm:w-[30px] sm:h-[30px]" />
                        Search with <span className="text-[#CB99C7]">AI</span>
                    </h1>
                    <div className="bg-gray-700 flex items-center rounded-full overflow-hidden shadow-lg relative w-full">
                        <input
                            type="text"
                            className="bg-transparent text-white flex-grow px-4 py-3 placeholder-gray-400 focus:outline-none text-sm sm:text-base"
                        ...
                        {input && (
                            <button className="absolute right-14 sm:right-16 bg-white rounded-full">
                                <img
                                    ...
                                    onClick={() => hanldeRecommendation(input)}
                                />
                            </button>
                        )}
                ...
                {recommendation.length > 0 ? (
                    ...
                    <h1 className="text-center text-xlsm:text-2xl mt-10 text-gray-400">Listening...</h1>
                ) : (
                    <h1 className="text-center text-xlsm:text-2xl mt-10 text-gray-400">No Courses Found Yet </h1>
                )}
            </div>
        </>
    );
