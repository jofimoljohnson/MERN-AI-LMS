...
const EditCourse = () => {
    ...
    const getCourseById = async () => {
        try {
            const result = await api.get(`/api/course/getcourse/${courseId}`);
    ...
    const handleEditCourse = async () => {
        ...
        try {
            const result = await api.put(`/api/course/editcourse/${courseId}`, formData);
            console.log(result.data);
    ...
    const handleRemoveCourse = async () => {
        setLoading1(true);
        try {
            const result = await api.delete(`/api/course/remove/${courseId}`);
            console.log(result.data);
            const filterCourses = courseData.filter((c) => c._id !== courseId);
            dispatch(setCourseData(filterCourses));
                ...
                <div className="bg-gray-50 p-6 rounded-md">
                    ...
                    <form className="space-y-6" onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div className="flex items-center justify-start gap-[15px]">
                            <button
                                className="bg-[#e9e8e8] hover:bg-red-200 text-black border-1 border-black cursor-pointer px-5 py-2 rounded-md"
                                onClick={() => navigate("/courses")}
                            >
                                Cancel
                            </button>
