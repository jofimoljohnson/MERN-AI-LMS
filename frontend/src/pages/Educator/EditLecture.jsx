...
const EditLecture = () => {
    ...
    const [isPreviewFree, seIsPreviewFree] = useState(false);
    ...
    const handleEditLecture = async () => {
        setLoading(true);
        try {
            const result = await api.put(`/api/course/editlecture/${lectureId}`, formData);
    ...
    const removeLecture = async () => {
        setLoading1(true);
        try {
            const result = await api.delete(`/api/course/removelecture/${lectureId}`);
            console.log(result.data);
            setLoading1(false);
        ...
        <>
            <div className="min-h-screen bg-gray-100 flex items-center justify-center p-4">
                <div className="w-full max-w-xl bg-white rounded-xl shadow-lg p-6 space-y-6">
                    ...
                    <button className="mt-2 px-4 py-2 bg-red-600  text-white  rounded-md hover:bg-red-700 transition-all text-sm" onClick={removeLecture} disabled={loading1}>
                      {
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex items-center gap-3">
                            <input
                                ...
                                id="isFree"
                                onChange={() => seIsPreviewFree((prev) => !prev)}
                            />
